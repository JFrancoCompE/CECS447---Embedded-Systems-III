...
//  SYSCTL_RCGC2_R |= 0x02; // activate port A
//  UART1_CTL_R &= ~0x01;      // disable UART
//	
//  UART1_IBRD_R = 54;//27;                    // IBRD = int(50,000,000 / (16 * 115,200)) = int(27.1267)
//  UART1_FBRD_R = 16;//8;                     // FBRD = int(0.1267 * 64 + 0.5) = 8
//                                        // 8 bit word length (no parity bits, one stop bit, FIFOs)
//  //UART1_IBRD_R = 81;// 38400 
//  //UART1_FBRD_R = 24;// 38400
//	
...
void UART1_Init(void){
  ...
  UART1_IBRD_R = 54;                    // IBRD = int(50,000,000 / (16 * 115,200)) = int(27.1267)
  UART1_FBRD_R = 16;                     // FBRD = int(0.1267 * 64 + 0.5) = 8
                                        // 8 bit word length (no parity bits, one stop bit, FIFOs)
  UART1_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);
  UART1_CTL_R |= 0x0301;       // enable rx,tx, and UART
    /*
  ...
  GPIO_PORTC_DEN_R |= 0x30;             // enable digital I/O on PB1-0
                                        // configure PB1-0 as UART
  GPIO_PORTC_PCTL_R = (GPIO_PORTC_PCTL_R&0xFF00FFFF)+0x00220000;
  GPIO_PORTC_AMSEL_R &= ~0x30;          // disable analog functionality on PB
}

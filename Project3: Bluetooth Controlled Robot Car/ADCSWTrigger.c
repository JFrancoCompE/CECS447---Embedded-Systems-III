...
void ADC_In1298(unsigned long *ain2, unsigned long *ain9, unsigned long *ain8, unsigned long *ain1){
  ...
  *ain8 = ADC0_SSFIFO2_R&0xFFF;    // 3C) read third result
	*ain1 = ADC0_SSFIFO2_R&0xFFF;    // 3C) read third result
...
void SysTick_Wait20Hz(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
    SysTick_Wait(2500000);  // wait 10ms (assumes 50 MHz clock)
  }
}
...
void ReadADCMedianFilter(unsigned long *ain2, unsigned long *ain9, unsigned long *ain8, unsigned long *ain1){
  ...
  *ain8 = median(ain8newest, ain8middle, ain8oldest);
  ...
  ain2oldest = ain2middle;
	ain9oldest = ain9middle; 
	ain8oldest = ain8middle;
  
	ain2middle = ain2newest; 
	ain9middle = ain9newest; 
	ain8middle = ain8newest;
}